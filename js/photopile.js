...
var photopile = (function() {
    ...
    // Thumbnails
    var numLayers         = 2;          // number of layers in the pile (max zindex)
    ...
    // Images
    var loading    = 'https://ws1.sinaimg.cn/large/006tNbRwly1fx4kck5ozxg300p00p3ye.gif';  // path to img displayed while gallery/thumbnails loads
            ...
            if ( self.isPickedUp ) {
                ...
                self.loadImage( thumb.getActiveImgSrc(), function() {
                    ...
                    $('body').bind('touchstart',function(e){
                        e.preventDefault();
                        window.startX = e.originalEvent.changedTouches[0].pageX,
                        window.startY = e.originalEvent.changedTouches[0].pageY;
                    });
                    $('body').bind('touchmove',function(e){
                        e.preventDefault();
                        //获取滑动屏幕时的X,Y
                        window.endX = e.originalEvent.changedTouches[0].pageX,
                        window.endY = e.originalEvent.changedTouches[0].pageY;
                        //获取滑动距离
                        distanceX = window.endX-window.startX;
                        distanceY = window.endY-window.startY;
                        //判断滑动方向
                        if(distanceX>10){
                            navigator.pickupPrev();
                            console.log('往右滑动');
